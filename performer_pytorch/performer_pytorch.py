...
def find_modules(nn_module, type):
    return [
        module for module in nn_module.modules() if isinstance(module, type)
    ]
...
class FastAttention(nn.Module):
    def __init__(self,
                 ...
                 qr_uniform_q=False):
        super().__init__()
        nb_features = default(nb_features,
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        (q, lq), (k, lk), (v, lv) = map(lambda t: (t[:, :gh], t[:, gh:]),
                                        (q, k, v))
...
class Performer(nn.Module):
    def __init__(self,
        ...
        local_attn_heads = local_attn_heads * depth if len(
            local_attn_heads) == 1 else local_attn_heads
        assert len(
            local_attn_heads
        ) == depth, 'tuple specifying number of local attention heads per depth must be equal to the total depth'
        assert all(
...
class PerformerLM(nn.Module):
    def __init__(self,
        ...
        self.performer = Performer(dim, depth, heads, local_attn_heads,
                                   local_window_size, causal, ff_mult,
                                   nb_features, reversible, ff_chunks,
                                   generalized_attention, kernel_fn,
                                   qr_uniform_q, use_scalenorm, use_rezero,
                                   ff_glu, ff_dropout, attn_dropout)
        self.norm = nn.LayerNorm(dim)
